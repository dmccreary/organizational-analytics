...
- [ ] **course-journey-map** (`docs/sims/course-journey-map/`)
    - `course-journey-map.js` uses `curveVertex(...)`, renamed to `splineVertex()` in v2 with changed anchor-point rules — rename to `splineVertex()`; drop the old duplicated first/last anchor points and rely on `endShape(CLOSE)` for a smooth closed loop.
- [ ] **end-to-end-pipeline** (`docs/sims/end-to-end-pipeline/`)
    - `end-to-end-pipeline.js` uses the old multi-control-point `bezierVertex(...)` call — v2 takes one control point per `bezierVertex()` call — chain multiple calls instead of packing several points into one; use `bezierOrder()` for a quadratic curve.
- [ ] **ml-workflow-pipeline** (`docs/sims/ml-workflow-pipeline/`)
